-- Punjab cluster coordinator scheduler: teacher scores table
CREATE TABLE IF NOT EXISTS punjab_teacher_scores (
  ...
  grade TEXT,
  subject TEXT,
  -- 6 Punjab observation indicators (raw scores)
...
CREATE POLICY "authenticated_read_punjab_teachers"
  ON punjab_teacher_scores
  FOR SELECT
  TO authenticated
  USING (true);

CREATE INDEX IF NOT EXISTS idx_punjab_teacher_scores_cluster
  ON punjab_teacher_scores (cluster_name);
