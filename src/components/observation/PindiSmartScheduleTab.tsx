...
import { useRawalpindiCoaching } from '@/hooks/useRawalpindiCoaching';
...
import { queueVisit, makePseudoObservation, syncPendingVisits } from '@/lib/rawalpindiOfflineQueue';
...
import {
  PINDI_CRITICAL_PCT,
  PINDI_ON_TRACK_PCT,
  PINDI_WEEKLY_VISIT_TARGET,
  PINDI_STAGNATION_OBS_MIN,
  PINDI_ESCALATION_OBS_MIN,
} from '@/domain/thresholds';

// Indicator max scores (sum = 51 = max_total_score) — same HOTS framework as Punjab
const PINDI_INDICATORS = [
...
function tierLabel(pct: number) {
  if (pct >= PINDI_ON_TRACK_PCT) return { label: 'On Track', className: 'text-green-700 bg-green-50 border-green-200' };
  if (pct >= PINDI_CRITICAL_PCT) return { label: 'Needs Follow-up', className: 'text-yellow-700 bg-yellow-50 border-yellow-200' };
...
function indicatorLevel(score: number, max: number): { label: 'Low' | 'Medium' | 'High'; className: string } {
  ...
  if (pct >= PINDI_ON_TRACK_PCT) return { label: 'High', className: 'text-green-700' };
  if (pct >= PINDI_CRITICAL_PCT) return { label: 'Medium', className: 'text-yellow-700' };
...
function readVisitedEntries(userId: string, fallbackDate: string): VisitedEntry[] {
  try {
    const raw = localStorage.getItem(`rawalpindi_visited_${userId}`);
    if (!raw) return [];
...
export default function PindiSmartScheduleTab({ onNewObservation }: Props) {
  ...
  const { teachers, loading, error, isOffline, lastSynced, loadTeachers } = useRawalpindiCoaching();
  ...
  const [flaggedTeachers, setFlaggedTeachers] = useState<Set<string>>(() => {
    ...
    try {
      const raw = localStorage.getItem(`rawalpindi_flagged_${user.id}`);
  ...
  const flagForRO = (teacherName: string) => {
    ...
    localStorage.setItem(`rawalpindi_flagged_${user.id}`, JSON.stringify([...updated]));
    toast.success(`${teacherName} flagged — raise with Regional Office at next sync`);
  };

  const getCycleStartDate = () => {
    if (!user?.id) return null;
    const stored = localStorage.getItem(`rawalpindi_cycle_start_${user.id}`);
  ...
  const markVisitedLocally = (teacherName: string) => {
    ...
    localStorage.setItem(`rawalpindi_visited_${user.id}`, JSON.stringify(updated));
  };
  ...
  const resetCycle = () => {
    ...
    if (existing) {
      const prev = JSON.parse(localStorage.getItem(`rawalpindi_cycle_history_${user.id}`) || '[]');
      prev.unshift({ startDate: existing.toISOString(), endDate: new Date().toISOString(), visitedCount, totalCount });
      localStorage.setItem(`rawalpindi_cycle_history_${user.id}`, JSON.stringify(prev.slice(0, 10)));
    }
    localStorage.setItem(`rawalpindi_cycle_start_${user.id}`, new Date().toISOString());
    localStorage.removeItem(`rawalpindi_visited_${user.id}`);
    toast.success('New cycle started');
  };

  const refresh = useCallback(() => {
    if (rawalpindiCluster) loadTeachers(rawalpindiCluster, user?.id ?? undefined);
  }, [rawalpindiCluster, loadTeachers, user?.id]);
  ...
  useEffect(() => {
    ...
    const handler = async () => {
      ...
      if (synced > 0) {
        ...
        if (rawalpindiCluster) loadTeachers(rawalpindiCluster, user.id);
      }
    };
    window.addEventListener('online', handler);
    return () => window.removeEventListener('online', handler);
  }, [user?.id, rawalpindiCluster, loadTeachers]);
        ...
        <CardContent className="flex flex-col items-center justify-center py-14 text-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground max-w-sm">
              Your account doesn't have a Rawalpindi cluster assigned. Please contact your administrator
  ...
  const critical = teachers.filter(t => t.overall_percentage < PINDI_CRITICAL_PCT);
  const watch = teachers.filter(t => t.overall_percentage >= PINDI_CRITICAL_PCT && t.overall_percentage < PINDI_ON_TRACK_PCT);
  const onTrack = teachers.filter(t => t.overall_percentage >= PINDI_ON_TRACK_PCT);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          <h2 className="font-display text-lg font-semibold text-foreground">{rawalpindiCluster}</h2>
            ...
            <CardContent className="p-4">
              ...
              <div className="flex items-start gap-3">
                <div className={`shrink-0 w-10 h-10 rounded-full border-2 flex items-center justify-center text-xs font-bold ${
                  nextTeacher.overall_percentage >= PINDI_ON_TRACK_PCT ? 'border-green-400 text-green-700 bg-green-50' :
                  nextTeacher.overall_percentage >= PINDI_CRITICAL_PCT ? 'border-yellow-400 text-yellow-700 bg-yellow-50' :
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <Card className={`glass-card border-l-4 ${visitedThisWeek >= PINDI_WEEKLY_VISIT_TARGET ? 'border-l-green-500' : visitedThisWeek >= 1 ? 'border-l-amber-500' : 'border-l-red-400'}`}>
          <CardContent className="p-3 text-center">
            <div className={`text-2xl font-bold ${visitedThisWeek >= PINDI_WEEKLY_VISIT_TARGET ? 'text-green-600' : visitedThisWeek >= 1 ? 'text-amber-600' : 'text-red-600'}`}>
              {visitedThisWeek}/{PINDI_WEEKLY_VISIT_TARGET}
            </div>
                  ...
                  <div key={s.school} className={`border rounded-lg p-3 ${bgColor}`}>
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className={`text-xs ${textColor} opacity-80`}>{s.visited}/{s.total} teachers visited</p>
                      </div>
        ...
        tier.length > 0 && (
          <div key={title} className="space-y-2">
            ...
            {tier.map(teacher => {
              ...
              const isStagnant = teacher.observation_count >= PINDI_STAGNATION_OBS_MIN && teacher.overall_percentage < PINDI_CRITICAL_PCT;
              const needsEscalation = teacher.observation_count >= PINDI_ESCALATION_OBS_MIN && teacher.overall_percentage < PINDI_CRITICAL_PCT;
                  ...
                  <CardContent className="p-4 space-y-3">
                    <div className="flex items-start gap-3">
                      <div className={`shrink-0 w-11 h-11 rounded-full border-2 flex items-center justify-center text-xs font-bold ${
                        teacher.overall_percentage >= PINDI_ON_TRACK_PCT ? 'border-green-400 text-green-700 bg-green-50' :
                        teacher.overall_percentage >= PINDI_CRITICAL_PCT ? 'border-yellow-400 text-yellow-700 bg-yellow-50' :
                        'border-red-400 text-red-700 bg-red-50'
                      }`}>
                              ...
                              <div className="h-1.5 bg-muted rounded-full overflow-hidden">
                                <div
                                  className={`h-full rounded-full transition-all ${
                                    pct >= PINDI_ON_TRACK_PCT ? 'bg-green-500' : pct >= PINDI_CRITICAL_PCT ? 'bg-yellow-500' : 'bg-red-500'
                                  }`}
                                  style={{ width: `${pct}%` }}
                                />
                              </div>
      ...
      {teachers.length === 0 && (
        <Card className="glass-card">
          <CardContent className="flex flex-col items-center justify-center py-14 text-center gap-3">
            ...
            <p className="text-sm text-muted-foreground">
              No teachers in {rawalpindiCluster} yet. Check back after data sync.
            </p>
          </CardContent>
        </Card>
      )}
