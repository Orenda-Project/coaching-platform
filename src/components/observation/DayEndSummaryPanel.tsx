...
export function DayEndSummaryPanel({
  ...
  const reportText = `📊 Daily Coaching Summary — ${new Date().toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })}

👨‍🏫 Coached by: ${coachName}
🏘️ Region: ${subRegion}

✅ Visits Completed: ${todayCompletedVisits.length}
📈 Average Score: ${avgScore}/4.0
🚀 Teachers Improving: ${improvingTeachers.length}

📋 Teachers Visited:
${todayCompletedVisits.map((obs) => {
  const score = obs.total_score?.toFixed(1) || '—';
  const grade = obs.neo_results?.grade || '—';
  return `  • ${obs.teacher_name} (${obs.school_name}) — Score: ${score}, Grade: ${grade}`;
}).join('\n')}
