...
const PAYLOAD: ScheduleVisitPayload = {
  observer_id: USER_ID,
  teacher_name: 'Hajra Bibi',
  school_name: 'GPS CHHAJI MAR',
  subject: 'Math',
  grade: 'Grade 3',
  topic: null,
  framework: 'HOTS',
...
describe('getPendingVisits', () => {
  ...
  it('returns empty array when localStorage is malformed', () => {
    localStorage.setItem(`punjab_offline_queue_${USER_ID}`, 'not-json');
    expect(getPendingVisits(USER_ID)).toEqual([]);
  });
});
